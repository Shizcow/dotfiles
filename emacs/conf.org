   ...
   (t
    (let* ((default-directory (file-name-as-directory shizcow/llama-temp-dir)))
  	...
  	"rm -rf llama.cpp && "
        ...
        "cd llama.cpp && "
        "cmake -B build -DBUILD_SHARED_LIBS=OFF -DCMAKE_INSTALL_PREFIX="(shell-quote-argument shizcow/llama-install-dir)" && "
	...
	(let ((uri (cdr (assoc 'location tid))))
	  ...
	  (if (not (file-exists-p dest-loc))
		...
		(cond ((shizcow/url-is-pdf uri)
		      ...
		      ;; arXiv papers are easy to look up
		      ((string-match-p "/arxiv\\." uri)
		       (progn (message "... from arXiv (grabbing indirect link)" uri)
			      (let* ((arxiv-slug (replace-regexp-in-string ".*/arXiv\\." "" uri))
				     (arxiv-pdf-url (concat "https://arxiv.org/pdf/" arxiv-slug)))
				(url-copy-file arxiv-pdf-url dest-loc t))))
		      ;; Otherwise, try to pirate the paper
		      ((shizcow/url-is-doi uri)
		       (progn (message "... via scihub (via doi)" uri)
				...
				;; Note: We advise scihub-fetch-callback
				;; later to allow for external browser
				;; download on failure.
				(scihub uri dest-loc))))
...
The above will attempt to download papers from SciHub, but can fail if the resource is not known to SciHub. In that case, we allow for eternal downloading. This automatically sets up:

#+begin_src elisp
(defun shizcow/external-download (doi filename)
  "Open DOI in browser, copy FILENAME to clipboard, and wait for manual download. Open when finished."
  (let ((doi-url (concat "https://doi.org/" doi)))
    (browse-url doi-url)
    (kill-new filename)
    (message "Opened DOI in browser. Path copied to clipboard: %s. Please download the PDF to this location. Emacs will resume once file is downloaded." filename)
    (let ((wait-time 1)) ;; seconds between checks
      (while (not (file-exists-p filename))
        (sleep-for wait-time)))
    (message "File downloaded correctly: opening %s..." filename)
    (org-link-open-as-file filename 'system)))

(defun shizcow/scihub-failure-hook (orig-fn status homepage query filename)
  "Advice to detect Sci-Hub failure and print a message."
  (condition-case err
      (funcall orig-fn status homepage query filename)
    (scihub-not-found
     (shizcow/external-download query filename)
     ))) ;; Re-raise the original error

;; Add the advice
(advice-add 'scihub-fetch-callback :around 'shizcow/scihub-failure-hook)
#+end_src
