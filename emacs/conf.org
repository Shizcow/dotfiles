...
(when (eq (buffer-file-name (window-buffer (minibuffer-selected-window))) nil) ; give babel a place to tangle
    (write-file (concat temporary-file-directory "emacs_conf_tmpwrite.org")))
...
(setq org-latex-pdf-process
      ...
      '("xelatex -8bit -shell-escape -interaction nonstopmode -output-directory %o %f"
        ...
        ; also check for table of contents changes -- if there's only a single run it fails
