...
The first is a function to help generate slugs for org roam files. This is mostly a fallback for files that don't have a proper method hard-coded yet.
#+begin_src elisp
...
#+RESULTS:
: shizcow/generate-new-slug-from

#+begin_src elisp
(shizcow/generate-new-slug-from nil nil '(1 2 3))
#+end_src
  ...
  (defun shizcow/org-roam-add-template-type (template-items &optional path-spec title-str additional-props)
    ...
    (setq template-items
	  (append template-items
		  (list :if-new	
			(list 'file+head (concat "%(shizcow/generate-new-slug-from "
						 (prin1-to-string path-spec)
						 " "
						 (prin1-to-string title-str)
						 " "
						 (prin1-to-string additional-props)
						 ")")
			      (concat ":PROPERTIES:
:%(identity shizcow/org-roam-enhanced-header-ver-name): %(shizcow/org-roam-enhanced-header-get-ver-str)
