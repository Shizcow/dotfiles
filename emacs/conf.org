    ...
    (let* ((default-directory (file-name-as-directory shizcow/llama-temp-dir)))
      ...
      (compile 
       (concat
        ...
        "rm -rf " (shell-quote-argument shizcow/llama-temp-dir)
        )
       t)
...
#+RESULTS:
| shizcow/llama-install-post-check | t |
...
** Interface
Now, a method of interfacing with the models:

#+begin_src elisp
(defun shizcow/with-llama (shortname prompt &optional timeout)
...
Returns all text if timeout occurs. Captures only stdout."
  ...
  (let* ((timeout (or timeout 5))
         ...
         (args (list "-m" model-path
                     "--prompt" prompt  ; Ensure correct spelling here
...
(shizcow/with-llama 
 "mistral-7b"
 "Write me a haiku about yak shaving. Output only the three lines of the haiku, absolutely nothing else." 10)
#+end_src
