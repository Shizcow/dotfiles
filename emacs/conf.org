...
window.onload = function(){
    // Fix strikethrough
    [...document.getElementsByTagName('sub')].forEach((el) => {
	if(el.children.length == 1 && el.children[0].tagName == 'SUB') {
	    el.outerHTML = '<strike>' + el.children[0].innerHTML + '</strike>';
	}
    });
	...
	} else if(el.innerHTML.startsWith('[ ] ')) {
	...
	if(!el.className)
            return;
	fetch('http://localhost:8069/babel-format?lang='
	      + el.className +'&data='
	      + btoa(el.innerHTML.replace(/^\\s+|\\s+$/g, '').replaceAll('\\t', '    ')).replaceAll('=', '~'))
...
that name."
    (interactive)
    (setq httpd-port 8069)
    (httpd-start)
    (impatient-mode 1)
    (imp-set-user-filter 'markdown-html)
    (cl-incf imp-last-state)
    
    (start-process "" nil "xdg-open" (concat "http://localhost:8069/imp/live/" (buffer-name))))
  )
#+end_src
* Circuits
